 ...
 * Origin Private File System so nothing large sits in memory. Optional English
...
import { type ResolvedSettings, type WorkerInbound } from '../types';
...
async function getOCRWorker(jobId: string, pages: number): Promise<any> {
  if (ocrWorker) return ocrWorker;
  send('progress', {
    id: jobId,
    progress: 12,
    message: 'Loading the English text recognition model for the first time',
  });
  if (!createOCRWorker) {
...
async function processJob({ id, file, settings }: ProcessRequest): Promise<void> {
  ...
  try {
    ...
    for (let page = 0; page < pages; page += 1) {
      ...
      if (settings.strategy !== 'flatten' && (analysis.usable || !settings.recognizeText)) {
        lastOriginalPage = page;
      }
    }
    for (let page = 0; page < pages; page += 1) {
      ...
      if (settings.recognizeText && !analysis.usable) {
        ...
        send('progress', {
          ...
          message: `No selectable text on page ${page + 1}; reading it in English`,
        });
    ...
    send('progress', {
      ...
      message:
        settings.strategy === 'flatten'
          ...
          : 'Recompressing images while preserving text',
    });
    ...
    if (opfsAvailable) {
      try {
        ...
        const bytes: Uint8Array = pyodide.FS.readFile(outputPath);
